<template>
  <div class="unbind-card bottom-safe-area">
    <div class="title">Set Trading Password</div>
    <div class="sub-title">
      <span v-if="!isAgain"
        >Enabling transaction function, please set transaction password</span
      >
      <span v-else>Please enter again</span>
    </div>
...
<script setup>
import { setTradePaswordApi } from "@/api/myApi";
...
const keyboardClick = (item) => {
  const key = isAgain.value ? "pwdAgain" : "pwd";
  const password = pwdMap[key];
  if (item.num != "del" && item.num) {
    if (isAgain.value && password.value.length >= 6) {
      return false;
    }
    pwdEncryption.value += "*";
    password.value += item.num;
    if (isAgain.value && password.value.length >= 6) {
      if (pwd.value !== pwdAgain.value) {
        showFailToast("Two passwords do not match,Please re-enter");
        pwd.value = "";
        pwdAgain.value = "";
        isAgain.value = false;
        pwdEncryption.value = "";
    ...
    if (pwdEncryption.value.length >= 6 && !isAgain.value) {
      isAgain.value = true;
      pwdEncryption.value = "";
    }
  }
  if (item.num == "del") {
    ...
    const pwdArr = password.value.split("");
    pwdArr.pop();
    password.value = pwdArr.join("");
  }
};

const setPassword = () => {
  const params = {
    bankPassword: pwd.value,
  };
  setTradePaswordApi(params).then(async (res) => {
    showSuccessToast("Set success");
